...
function Property(id, required, label, value, sample, desc, isCalendar, isTime, isSection, isTextArea, InputWidth, isLookup, LookupFactory, isHTML, targetObj, targetObjvalue, isNumber, type, options)
{
...
// in the CategorySink.
CategorySink.prototype.addProperty = function(id, required, label, value, sample, desc, type, options, isCalendar, isTime, isSection, isTextArea, InputWidth, isLookup, LookupFactory, isHTML, targetObj, targetObjvalue, isNumber)
{
  // alert("1: " + id + " 2: " + required+ " 3: " + label+ " 4: " + value+ " 5: " + sample+ " 6: " + desc+ " 7: " + type+ " 8: " + options+ " 9: " + isCalendar+ " 10: " + isTime+ " 11: " + isSection+ " 12: " + isTextArea+ " 13: " + InputWidth+ " 14: " + isLookup+ " 15: " + LookupFactory+ " 16: " + isHTML+ " 17: Target / " + targetObj+ " 18: Value / " + targetObjvalue);
  rxLt = new RegExp("&lt;", "gi");
  rxGt = new RegExp("&gt;", "gi");
...
var p = new Property(id, required, label, value, sample, desc, type, options, isCalendar, isTime, isSection, isTextArea, InputWidth, isLookup, LookupFactory, isHTML, targetObj, targetObjvalue, isNumber);
  ...
  else
  {
    ...
    for (e = 0; e < this.properties.length; e++)
    {
      ...
      if(prop.isHTML == "1") {
        ...
        ntr.setAttribute("align", "left");
        var ntd = document.createElement("td");
        ntd.setAttribute("width", "100%");
        ntd.setAttribute("colSpan", cols);
        ntd.setAttribute("style", "font-size: 10px;");
...
// 20131028 bmalz @ e-xim {
  ...
  if(!prop.required)
    ...
    labelDiv.className = "requiredlabeltext";
  ...
  if ( typeof prop.type == "string" && prop.type == "SELECT" ) 
  {
    ...
    k++;
          ...
          if(prop.targetObj != "" && prop.targetObjvalue != "") {
            onChg = 'ChangeObjectState(this, "' + prop.targetObj + '", "' + prop.targetObjvalue + '", "SELECT");';
            disabledElements.push(prop.targetObj);
          }

          s.onchange = new Function(onChg);
          // var jq_s = jq(s);
          // jq_s.change(function() {
          //   if(jq(this).val() != prop.targetObjvalue) {
          //     jq("[name=" + prop.targetObj + "]").attr("disabled", true);
          //   } else {
          //     jq("[name=" + prop.targetObj + "]").attr("disabled", false);
          //   }
          // });
...
else
{
          ...
          if ( typeof prop.type == "string" && prop.type == "CHECKBOX" )
          {
            ...
            var onC = "detailSetPcbFlag(this)";
            if(prop.targetObj != "" && prop.targetObjvalue != "") {
              onC = 'ChangeObjectState(this, "' + prop.targetObj + '", "' + prop.targetObjvalue + '", "CHECKBOX");';
              disabledElements.push(prop.targetObj);
            }
          ...
          else
          {
            if(prop.isLookup == "1") {
              ...
              if(prop.LookupFactory == "cnt") {
                LookupString = "<table><tr style=\"vertical-align:top!important\"><td style=\"font-family:Verdana, Arial, helvetica, sans-serif;color:#333333;font-size: 0.6em;\" ><input type=\"radio\" name=\"prop" + String(e) +".userrb\" value=\"lookup\" checked onclick=\"SetDisplay('prop" + String(e) + ".customuserboxname','none');SetDisplay('prop" + String(e) + ".value_combo_name','inline');SetDisplay('prop" + String(e) + ".value_lookup_name','inline');ClearUserData('prop" + String(e) + ".customuserboxid','SET.prop" + String(e) + ".value');\">Użytkownik SDM</input></td></tr><tr><td><a id=\"prop" + String(e) + ".value_lookup_name\" name=\"prop" + String(e) + ".value_lookup_name\" href=\"javascript:lookForUserCstCat('prop" + String(e) + ".value', 'main_form', 'cnt','' )\" title=\"Wybierz użytkownika\"><img class=\"dtl_img_attr\" src=\"/CAisd/img/lookup.gif\" alt=\"Wybierz  \" style=\"margin-left:10px;\"><span class=\"lookup\">Wybierz użytkownika</span></a></td></tr>";
                LookupString += "<tr><td><input id=\"prop" + String(e) + ".value_ID_combo_name\" name=\"prop" + String(e) + ".value_combo_name\" type=\"text\" size=\"18\" onchange=\"cleanUser('prop" + String(e) + ".value'); if (this.value != '') {lookForUserCstCat('" + _dtl.currID + "', 'main_form', 'cnt','');}\" style=\"margin-left:10px;\"><input type=\"hidden\" id=\"SET.prop" + String(e) + ".value\" name=\"SET.prop" + String(e) + ".value\"><input type=\"hidden\" id=\"KEY.prop" + String(e) + ".value\" name=\"KEY.prop" + String(e) + ".value\"><input type=\"hidden\" id=\"prop" + String(e) + ".value_lname\" name=\"prop" + String(e) + ".value_lname\"><input type=\"hidden\" id=\"prop" + String(e) + ".value_fname\" name=\"prop" + String(e) + ".value_fname\"><input type=\"hidden\" id=\"prop" + String(e) + ".value_mname\" name=\"prop" + String(e) + ".value_mname\"></td></tr><tr><td style=\"font-family:Verdana, Arial, helvetica, sans-serif;color:#333333;font-size: 0.6em;\"><input type=\"radio\" name=\"prop" + String(e) +".userrb\" value=\"notlookup\" onclick=\"SetDisplay('prop" + String(e) + ".value_combo_name','none');SetDisplay('prop" + String(e) + ".value_lookup_name','none');SetDisplay('prop" + String(e) + ".customuserboxname','inline');SetChecked('prop" + String(e) + ".customuserboxname','false');ClearUserData('prop" + String(e) + ".value_ID_combo_name','SET.prop" + String(e) + ".value');\">Użytkownik dowolny</input><td></tr><tr><td><textarea rows=\"3\" cols=\"50\" id=\"prop" + String(e) + ".customuserboxid\" name=\"prop" + String(e) + ".customuserboxname\" style=\"display:none;margin-left:10px;\" onchange=\"SetUserData('prop" + String(e) + ".customuserboxid','SET.prop" + String(e) + ".value');\" ></textarea></td></tr></table>";
              } else {
              ...
              if(prop.isTextArea == '1') {
                ...
                if(prop.InputWidth != "")
                {
                  if(!prop.isNumber) {
                    ...
                    inputstring = "<input id=\"" + _dtl.currID + "\" tabindex=\"" + _dtl.tabIndex + "\" name=\"SET.prop" + String(e) + ".value\" title=\""+ prop.value + "\" size=\"" + prop.InputWidth + "\" onchange=\"validateNumber(this);\">";
                  }
                }
                else 
                {
                  if(!prop.isNumber) {
                    ...
                    inputstring = "<input id=\"" + _dtl.currID + "\" tabindex=\"" + _dtl.tabIndex + "\" name=\"SET.prop" + String(e) + ".value\" title=\""+ prop.value + "\" onchange=\"validateNumber(this);\">";
        ...
        if (typeof prop.sample == "string" && prop.sample.length > 0 && prop.isHTML != "1")
        {
          ...
          if (!_browser.isMAC)
          {
            //2013-11-11 bula @ e-xim : wyciete bo nie dziala ja nalezy :(
/*            td = document.createElement("td");
            newCol.appendChild(td);
      ...
      for(var i=0; i<disabledElements.length; i++) {
        jq('*[name="' + disabledElements[i] + '"]').attr("disabled", true);
      }
...
function ChangeObjectState(thisobj, obj, objvalue, type)
{
  if(type == "CHECKBOX") {
    SetDisability(String(thisobj.checked), obj, String(objvalue));
    detailSetPcbFlag(thisobj);
  } else if (type == "SELECT") {
    SetDisability(String(thisobj.value), obj, String(objvalue));
  }
}
...
function validateNumber(inputId)
{
  if(typeof inputId != "undefined") {
    ...
    if(isNaN(value))
    {
      detailReportValidation(inputId, 1, "Niepoprawny format liczby");
    }
    else
    {
      detailReportValidation(inputId, 0);
    }
  }
}
...
{
  jq('*[name="' + obj + '"]').css("display", value);
}

function SetChecked(radiobtn, value)
{
  jq('*[name="' + radiobtn + '"]').attr('checked',value);
}

function ClearUserData(field, hiddenInput)
{
  var f = document.getElementById(field);
  f.value = '';
  var p = document.getElementById(hiddenInput);
  p.value = '';
}

function SetUserData(field, hiddenInput)
{
  var f = document.getElementById(field);
  var p = document.getElementById(hiddenInput);
  p.value = f.value;
...
function SetDisability(source, target, value) {
  if(source != value) {
    jq('*[name="' + target + '"]').attr("disabled", true);
  } else {
    jq('*[name="' + target + '"]').attr("disabled", false);
  }
}
